{{
  ...
  0.2: KP - basic implementation, without watchdog
...
var 
  ...
  Long temp           'Long to save calculation values temporarily
  Long remainderR     'Long to save the remainder of the setpoint calculation
  Long remainderL     'Long to save the remainder of the setpoint calculation
...
pri move(vx, rot) | setpL, setpR
  ...
  'setpL := temp/100
  'remainderL := remainderL + (temp - (setpL*100))

  'if remainderL > 100
  '  setpL := setpL + (remainderL/100)
  '  remainderL:= remainderL - ((remainderL/100)*100)
  ...
  'setpR := temp/100
  'remainderR := remainderR + (temp - (setpR*100))

  'if remainderR > 100
  '  setpR := setpR + (remainderR/100)
  '  remainderR:= remainderR - ((remainderL/100)*100)
